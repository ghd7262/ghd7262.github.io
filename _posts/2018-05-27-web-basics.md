...
TCP/IP
  ...
  - HTTP request/response를 한 기기가 다른 기기에게 보내면 HTTP에 의해 약속된 형식으로 작성이 되고 TCP/IP에 의해 통신에 기본적으로 필요한 헤더가 추가적으로 붙게된다. 예를 들면 상대 기기를 찾아가기 위해 필요한 ip address와 보내는 데이터의 무결성을 확인하기 위해 필요한 packet '일련번호' 같은 것이 있다.
