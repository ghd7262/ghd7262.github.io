...
servlet 들어오는 HTTP 요청을 받고 응답을 작성해서 보내는 역할을 한다. 또, CGI와는 달리 요청이 들어올 때마다 새로운 프로세스를 생성하지 않기 때문에 서버의 효율을 높였다. 또한 자바 클래스이기 때문에 모든 자바 클래스 라이브러리를 사용할 수 있다.

> **요약하자면, servlet의 역할은 요청을 받아서 해석하고, 해석된 데이터에 따라 (웹 어플리케이션에 의해) 동적으로 선택된 리소스를 보내는 것이다.**
